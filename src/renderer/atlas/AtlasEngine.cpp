...
template<typename T>
constexpr AtlasEngine::vec2<T> yolo_vec2(COORD val)
{
    return { yolo_narrow<T>(val.X), yolo_narrow<T>(val.Y) };
}

template<typename T>
constexpr AtlasEngine::vec2<T> yolo_vec2(SIZE val)
{
...
[[nodiscard]] HRESULT AtlasEngine::SetWindowSize(const SIZE pixels) noexcept
{
    ...
    if (auto newSize = yolo_vec2<u16>(pixels); _api.sizeInPixel != newSize && newSize != u16x2{})
    {
...
{
    {
        ...
        const auto pxOptimal = size_t(std::ceil(std::sqrt(double(areaRequired))));
...
void AtlasEngine::_setShaderResources()
{
...
void AtlasEngine::_updateConstantBuffer()
{
    const_buffer data;
    data.viewport.x = 0;
    data.viewport.y = 0;
